...
class StatisticsProvider with ChangeNotifier {
  ...
  Future<void> loadStatistics(BuildContext context) async {
    ...
    try {
      ...
      await loadDailyStats(userId.toString());
      await loadWeeklyStats(userId.toString());
      await loadMonthlyStats(userId.toString());
      await loadProgressStats(userId.toString());
  ...
  Future<void> loadDailyStats(String userId) async {
    try {
  ...
  Future<void> loadProgressStats(String userId) async {
    try {
      final response = await http.get(
      ...
      if (response.statusCode == 200) {
        final List<dynamic> progressData = json.decode(response.body);
        _learningProgress = progressData.cast<Map<String, dynamic>>();
      }
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Öğrenme ilerlemesi yüklenirken hata oluştu: ${e.toString()}'),
          backgroundColor: Colors.red,
        ),
      );
    }
